...
final class Person {
    ...
    // MARK: - 공유 문서 링크
    /// 멘티와 공유하는 문서 링크들 (iCloud Numbers, Google Sheets 등)
    /// 형식: [{"title": "목표 시트", "url": "https://...", "type": "numbers"}]
    @Attribute
    var sharedDocumentLinks: [String] = []
    ...
    /// 이 사람에게 할당된 태그들 (기수, 상태, 분야 등)
    /// nullify 삭제: 태그가 삭제되어도 사람은 유지됨
    @Relationship(deleteRule: .nullify, inverse: \PersonTag.people)
    var tags: [PersonTag] = []
    
    /// 통합 기록들 (InteractionRecord, ConversationRecord, MeetingRecord, QuickMemoArchive를 통합)
    /// cascade 삭제: 사람이 삭제되면 관련 기록들도 모두 삭제됨
    @Relationship(deleteRule: .cascade, inverse: \Record.person)
    var records: [Record] = []
...
extension Person {
    ...
    func getBadgeDetails() -> [BadgeDetail] {
        ...
        #if os(macOS)
        let criticalTabIndex = 3
        let actionsTabIndex = 3
        let importantTabIndex = 2
        let criticalColor = NSColor.systemRed
        let actionColor = NSColor.systemBlue
        let importantColor = NSColor.systemYellow
        #else
        let criticalTabIndex = 0
        let actionsTabIndex = 0
        let importantTabIndex = 1
        let criticalColor = Color.red
        let actionColor = Color.blue
        let importantColor = Color.yellow
        #endif
...
// MARK: - Record (통합 기록) 관리 메서드들
extension Person {
    
    /// 새로운 통합 기록 추가
    /// - Parameters:
    ///   - contactType: 접촉 타입 (선택 - nil이면 메모만)
    ///   - content: 기록 내용
    ///   - notes: 추가 메모
    ///   - tags: 태그들
    ///   - date: 기록 날짜
    ...
    ///   - location: 장소
    ///   - isImportant: 중요 표시
    ///   - audioFileName: 음성 파일명
    ///   - transcription: 음성 텍스트 변환 결과
    ///   - imageDataArray: 이미지 데이터 배열
    /// - Returns: 생성된 Record 객체
    @discardableResult
    func addRecord(
    ...
    /// 모든 통합 기록을 날짜 역순으로 정렬하여 반환
    func getAllRecordsSorted() -> [Record] {
        return records.sorted { $0.date > $1.date }
    }
    
    /// 특정 접촉 타입의 기록들 반환
    func getRecords(ofContactType contactType: ContactType) -> [Record] {
        return records
            .filter { $0.contactType == contactType }
            .sorted { $0.date > $1.date }
    }
    
    /// 특정 태그가 포함된 기록들 반환
    func getRecords(withTag tag: RecordTag) -> [Record] {
        return records
            .filter { $0.tags.contains(tag) }
            .sorted { $0.date > $1.date }
    }
    ...
    /// 중요 표시된 기록들 반환
    func getImportantRecords() -> [Record] {
        return records
            .filter { $0.isImportant }
            .sorted { $0.date > $1.date }
    }
    
    /// 미해결 기록들 반환 (약속, 질문 태그가 있고 isResolved가 false인 것들)
    func getUnresolvedRecords() -> [Record] {
        return records
            .filter { !$0.isResolved && ($0.tags.contains(.promise) || $0.tags.contains(.question)) }
            .sorted { $0.date < $1.date }  // 오래된 것부터
    }
    ...
    /// 기록 통계 정보 반환
    func getRecordStatistics() -> [String: Int] {
        let total = records.count
        let resolved = records.filter { $0.isResolved }.count
        let unresolved = getUnresolvedRecords().count
        let important = records.filter { $0.isImportant }.count
        let withAudio = records.filter { $0.hasAudio }.count
        
        var byContactType: [String: Int] = [:]
        for type in ContactType.allCases {
            byContactType[type.rawValue] = getRecords(ofContactType: type).count
        }
        
        var byTag: [String: Int] = [:]
        for tag in RecordTag.allCases {
            byTag[tag.rawValue] = getRecords(withTag: tag).count
        }
    ...
    /// 미해결 기록이 있는지 확인
    var hasUnresolvedRecords: Bool {
        return records.contains { !$0.isResolved && ($0.tags.contains(.promise) || $0.tags.contains(.question)) }
    }
    
    /// 가장 최근 통합 기록 날짜
    var mostRecentRecordDate: Date? {
        return records.map { $0.date }.max()
    }
}

// MARK: - Notification Names
extension NSNotification.Name {
    static let importantRecordingAdded = NSNotification.Name("importantRecordingAdded")
}
