    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            TabView(selection: $selectedTab) {
                ...
                // 관계도 탭 (새로 추가!)
                MacRelationshipGraphTab(person: person)
                    .tabItem {
                        Label("관계도", systemImage: "point.3.connected.trianglepath.dotted")
                    }
                    .tag(3)

                // 공유 문서 탭
                MacSharedDocumentsTab(person: person)
                    .tabItem {
                        Label("문서", systemImage: "doc.richtext")
                    }
                    .badge(person.sharedDocuments.count)
                    .tag(4)
