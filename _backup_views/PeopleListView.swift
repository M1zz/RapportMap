...
struct PeopleListView: View {
    ...
    @State private var showingQuickVoiceMemo = false
    ...
    private func applyFilter(_ filterCase: FilterCase, to people: [Person]) -> [Person] {
        ...
        case .tags:
            return applyTagsFilter(to: people)
            
        case .recentInteraction:
            return applyRecentInteractionFilter(to: people)
            
        case .dateRange:
            return applyDateRangeFilter(to: people)
        }
    }
    
    // MARK: - Individual Filter Methods
    
    /// 🆕 강화된 검색 필터 - 검색 범위에 따라 다르게 동작
    ...
    private func matchesMemo(_ person: Person, query: String) -> Bool {
        // 빠른 메모
        if person.quickMemo.localizedCaseInsensitiveContains(query) {
            return true
        }
        
        // 아카이브된 메모들
        if person.archivedMemos.contains(where: { memo in
        ...
        // 상호작용 기록의 노트
        if person.interactionRecords.contains(where: { record in
    ...
    // 🆕 최근 상담 필터
    private func applyRecentInteractionFilter(to people: [Person]) -> [Person] {
        guard let days = filterOptions.recentInteractionDays else { return people }
        ...
        return people.filter { person in
            // 상호작용 기록 확인
            let hasRecentInteraction = person.interactionRecords.contains { record in
                record.date >= cutoffDate
            }
            
            // 미팅 기록 확인
            let hasRecentMeeting = person.meetingRecords.contains { record in
                record.date >= cutoffDate
            }
    ...
    // 🆕 날짜 범위 필터
    private func applyDateRangeFilter(to people: [Person]) -> [Person] {
        guard filterOptions.dateRangeFilter != .all else { return people }
        ...
        if filterOptions.dateRangeFilter == .custom {
            // 직접 설정한 날짜 범위
            guard let fromDate = filterOptions.customDateFrom else { return people }
            cutoffDate = fromDate
        ...
        return people.filter { person in
            // 관계 시작일 기준
            if person.relationshipStartDate >= cutoffDate {
                return true
            }
            
            // 또는 최근 상호작용이 있는 경우
            if let lastInteraction = person.mostRecentInteractionDate,
               lastInteraction >= cutoffDate {
                return true
            }
    ...
    private func applyTagsFilter(to people: [Person]) -> [Person] {
        guard !filterOptions.selectedTagIDs.isEmpty else { return people }
    ...
    private func applySorting(to people: [Person]) -> [Person] {
        ...
        case .relationshipStart:
            sorted = people.sorted { p1, p2 in
                return p1.relationshipStartDate > p2.relationshipStartDate
            }
        }
    ...
    var body: some View {
        NavigationStack {
            ...
            #if os(iOS)
            .fullScreenCover(isPresented: $showingQuickVoiceMemo) { quickVoiceMemoSheet }
            #endif
    ...
    #if os(iOS)
    private var quickVoiceMemoSheet: some View {
        QuickVoiceMemoView()
    }
    #endif
...
struct PersonCard: View {
    ...
    private func getInteractionItems() -> [AnyView] {
        ...
        // 태그 표시 (최대 3개)
        let displayTags = Array(person.tags.prefix(3))
        for tag in displayTags {
            items.append(AnyView(
                TagBadge(tag: tag, isCompact: true)
            ))
        }
